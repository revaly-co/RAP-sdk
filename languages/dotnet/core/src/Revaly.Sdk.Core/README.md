...
namespace YourProject
{
    public class Program
    {
        ...
        public static IHostBuilder CreateHostBuilder(string[] args) => Host.CreateDefaultBuilder(args)
          ...
          {
              // The API authenticates with an API key sent in the Authorization header using
              // the `ApiKey` scheme prefix:  Authorization: ApiKey <your api key>
              // The prefix argument below is required — ApiKeyToken defaults to "Bearer ",
              // which the API rejects. (The prefix is concatenated verbatim: keep the trailing space.)
              ApiKeyToken token = new("<your api key>", ClientUtils.ApiKeyHeader.Authorization, prefix: "ApiKey ");
