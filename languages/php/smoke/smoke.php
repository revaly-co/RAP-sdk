...
$scenarios = [
    ...
    'charge-declined' => function () use ($client, $declinedId, $routingId, &$lastCorrelation): string {
        ...
        $transaction = $client->charge(buildCharge($declinedId, DECLINE_PAN, '2027', $routingId));
        if (($transaction->getTransactionId() ?? '') === '') {
        ...
        if ($transaction->getTransactionStatus() !== 2) {
            throw new SmokeFailure(sprintf('expected transactionStatus=2 (declined), got %s — the target gateway must be one where this decline PAN declines', var_export($transaction->getTransactionStatus(), true)));
        }
        if (($lastCorrelation ?? '') === '') {
