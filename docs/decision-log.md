...
| Stage-6 rollout | Registry publish ships **dark** on every release tag (rehearsal + flip-readiness report, no registry contact); flip is **double-keyed** (repo variable + guard-removal PR); Packagist via generated artifact-tree mirror; GitHub releases persist post-GA as provenance anchor | 2026-08-03 | SC squad | ADR-SDK-031 |
| Public-repo history | Fresh-repo cutover: current repo → private `RAP-sdk-archive`; new `revaly-co/RAP-sdk` reclaims the name with a `filter-repo`-sanitized history transplant (Variant B); old releases stay archived, versions continue; executes as flip-runbook gate 3 | 2026-08-03 | Leadership (Dimitri) + SC squad | ADR-SDK-032 |
...
| OQ-12 | Apache-2.0 (Charles); Legal ratifies before first publish; copyright Revaly — **Legal-ratified in writing, recorded 2026-08-06** (ADR-SDK-019 § Ratification record) | 2026-07-10 | Leadership + Legal | ADR-SDK-019 |
