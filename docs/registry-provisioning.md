...
ID-prefix reservation **submitted 2026-07-31** — resubmission from the account-registered
owner mailbox required, see table; PyPI org approval pending). Publish still embargoed.
...
| NuGet | `Revaly.Sdk` + `Revaly.Sdk.Core` | ✅ org (2026-07-17; access confirmed 2026-07-30) | **`Revaly.*` ID-prefix reservation** — request **submitted 2026-07-31**; NuGet support replied the same day: the request must originate **from the email registered to the NuGet.org account** (an individual mailbox is rejected; sender-identity check, not a substance rejection) → **resend from the owner group mailbox**, then the days-to-weeks review clock starts — the live long-lead act | Trusted-publishing policy — created close to publish day (policies on private repos auto-expire after 7 unused days), by a leadership account that is an org member |
...
| **NuGet ID-prefix (`Revaly.*`) reservation** — submitted 2026-07-31; per NuGet's same-day reply, **resend from the account-registered owner mailbox**, then days-to-weeks review | SC squad | Before first NuGet publish; resend immediately |
