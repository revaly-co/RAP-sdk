...
| Stage-4 smoke secrets (interim) | GitHub environment **`staging`**: `RAP_SMOKE_BASE_URL`, `RAP_SMOKE_API_KEY`, `RAP_SMOKE_GATEWAY_ROUTING_ID` | ADR-SDK-024 implementation record. Keys borrowed from the platform's staging E2E pool (scope lives on the key — prefer sandbox-scoped for GA parity); never logged. `RAP_SMOKE_FAULT_INJECT` is a **`staging` environment variable** (value `pre-dispatch` today), not hardcoded workflow env — so the GA retarget is provisioning-only: **delete the variable** and repoint the three secrets, no workflow edit. All three secrets are **provisioned and currently point at Backbone staging**. The guard is fail-closed on all three (the routing id is required, not optional). **At retarget** (`../docs/prod-sandbox-validation.md`, 2026-07-25): the injector is **inert on the production path** — verified, the injected charge simply approves — so removing the var also removes live coverage of the `503 + not_processed` row (moves to mock-only, accept in writing per ADR-SDK-024); and `RAP_SMOKE_GATEWAY_ROUTING_ID` becomes **required, not optional** — without it prod-sandbox charges land in the `flexpay_declined` sink (responseCode 50130) |
